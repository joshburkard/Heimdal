# Functional Tests for Get-HeimdalDeviceRiskScore
# Tests the Get-HeimdalDeviceRiskScore function behavior and return values
...
BeforeAll {
    ...
    $script:TestGetDeviceData = $script:TestData['Get-HeimdalDeviceRiskScore']

    Connect-Heimdal @ConnectData
}

Describe "Get-HeimdalDeviceRiskScore Function Tests" -Tag "Integration", "Connection" {
    Context "Test Data Validation" {

        It "Should have test data defined in declarations.ps1" {
            # Assert
            $script:TestGetDeviceData | Should -Not -BeNullOrEmpty
            $script:TestData.ContainsKey('Get-HeimdalDeviceRiskScore') | Should -Be $true
        }
        ...
        It "Should output test data for verification" {
            # Output test data
            Write-Host "`n=== Test Data for Get-HeimdalDeviceRiskScore ===" -ForegroundColor Cyan
    ...
    Context 'Connection Required' {
        It 'should throw when Connect-Heimdal has not been called' {
            ...
            { Get-HeimdalDeviceRiskScore @params } | Should -Throw
    ...
    Context "Get Device Risk Score by ClientInfoId" {
        It "Should retrieve device risk score for a valid ClientInfoId" {
            # Arrange
            $params = $script:TestGetDeviceData.ByClientInfoId

            # Act
            $result = Get-HeimdalDeviceRiskScore @params
